...
describe('authentication middlewares', () => {
    ...
    describe('/auth/register', () => {
        it('should allow to create a new user, returning the new username and session id and set a session id cookie', async () => {
            ...
            expect(await User.findAll({ where: { username: 'auth-test-user' } })).to.be.of.length(1);
        });
        ...
        it('should fail to create a new user if the username is already taken', async () => {
            await request(app).post('/api/auth/register').send({ username: 'auth-test-user', password: '12345679010' });
            expect(await User.count()).to.equal(2);
            const res = await request(app)
                .post('/api/auth/register')
        ...
        it('should fail if no username is provided', async () => {
            const res = await request(app).post('/api/auth/register').send({ password: '123456790' });
        ...
        it('should fail if no password is provided', async () => {
            const res = await request(app).post('/api/auth/register').send({ username: 'auth-test-user' });
    ...
    describe('/auth/login', () => {
        ...
        it('should fail to login on wrong username', async () => {
            const user_data = { username: 'auth-test-user', password: '12345678' };
            await request(app).post('/api/auth/register').send(user_data);
            const res = await request(app).post('/api/auth/login').send({ username: 'wrong', password: '12345678' });
            expect(res.status).to.equal(403);
            expect(res.body).to.deep.equal({
                status: 'error',
                error: 'Forbidden'
        ...
        it('should fail if no password is provided', async () => {
            const res = await request(app).post('/api/auth/login').send({ username: 'auth-test-user' });
    ...
    describe('/auth/whoami', async () => {
        it('should return username and session timeout if the user is authenticated', async () => {
            ...
            const res = await request(app).get('/api/auth/whoami').set('Cookie', [sessionId]).expect(200);
    ...
    describe('/auth/logout', () => {
        it('should logout if the user is authenticated', async () => {
            ...
            await request(app).get('/api/auth/logout').set('Cookie', [sessionId]).expect(200, { status: 'success' });
            await request(app).get('/api/auth/whoami').expect(401, {
                status: 'error',
                error: 'Unauthorized'
            });
