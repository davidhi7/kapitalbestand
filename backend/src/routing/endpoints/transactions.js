...
const categoryShopIdValidatorMiddleware = (categoryOptional = false) => {
    if (categoryOptional) {
        return [
            body('CategoryId')
                .custom((value, { req }) => categoryController.validateId(req.session.user, value, false))
                .optional(),
            body('ShopId')
                .custom((value, { req }) => shopController.validateId(req.session.user, value, true))
                .optional()
        ];
    }
    return [
        body('CategoryId').custom((value, { req }) => categoryController.validateId(req.session.user, value, false)),
        body('ShopId')
            .custom((value, { req }) => shopController.validateId(req.session.user, value, true))
            .optional()
    ];
...
router.get(
    ...
    query('dateFrom')
        .isDate()
        .custom((dateFrom, { req }) => checkDateLowerThan(dateFrom, req.query.dateTo))
        .toDate()
        .optional(),
    ...
    query('amountFrom')
        .isInt()
        .toInt()
        .custom((amountFrom, { req }) => checkNumberLowerThan(amountFrom, req.query.amountTo))
        .optional(),
...
router.get('/oneoff/:id', param('id').isInt({ min: 0 }).toInt(), (req, res, next) =>
    asyncEndpointWrapper(req, res, next, async () => {
...
router.post(
    ...
    categoryShopIdValidatorMiddleware(false),
    (req, res, next) =>
        asyncEndpointWrapper(req, res, next, async () => {
...
router.get(
    ...
    query('monthFrom')
        .matches(/\d{4}-\d{1,2}/)
        .custom((monthFrom, { req }) => checkDateLowerThan(monthFrom, req.query.monthTo))
        .toDate()
        .optional(),
    query('monthTo')
        .matches(/\d{4}-\d{1,2}/)
...
router.get('/monthly/:id', param('id').isInt({ min: 0 }).toInt(), (req, res, next) =>
    asyncEndpointWrapper(req, res, next, async () => {
...
router.post(
    ...
    body('monthFrom')
        .matches(/\d{4}-\d{1,2}/)
        .custom((monthFrom, { req }) => checkDateLowerThan(monthFrom, req.body.monthTo)),
    body('monthTo')
        .matches(/\d{4}-\d{1,2}/)
        .optional(),
    body('amount').isInt({ gt: 0 }).toInt(),
